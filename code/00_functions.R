...
# curvature_dAPC <- function(amin=20, amax=70, pmin=1990, pmax=2016, gr=1, r = "h"){
#   amax <- amin - 1 + (floor((amax-amin+1)/gr))*gr
#   pmax <- min(pmax, 2016)
#   pmax <- pmin - 1 + (floor((pmax-pmin+1)/gr))*gr
#   
#   data0 <- db %>% 
#     filter(year>=pmin, year<=pmax, age>=amin, age<=amax, race==r) %>% 
#     select(age, year, e, d)
#   
#   data1 <- data0 %>% 
...
#       Rc <- bs(c0, knots = attr(MC, "knots"), Boundary.knots = attr(MC, 
